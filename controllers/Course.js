...
// Ensure courseId is extracted correctly as a string
exports.publishCourse = async (req, res) => {
  try {
    ...
    if (course.instructor.toString() !== req.userInfo.userId) {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "All required course fields must be completed before publishing",
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "Instructor profile details are incomplete",
      });
    }
